...
class CameraThread:
    ...
    def start(self) -> bool:
        ...
        if not self.cap.isOpened():
            print(f"[{self.cam_name} ERROR] Gagal membuka kamera {self.source}!")
            self.is_connected = False
            return False

        # Set resolusi & FPS optimal jika didukung oleh hardware kamera
        ...
        self._thread.start()
        print(f"[{self.cam_name}] Berhasil terhubung! Capture loop berjalan @ {self.target_fps} FPS.")
        return True
